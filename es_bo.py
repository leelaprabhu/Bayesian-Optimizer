...
def f(Xa):
    ...
    ret=eng.code2(1,param)
    X_gp=numpy.row_stack((X_gp,Xa))
    y_gp=np.concatenate((y_gp,[ret]))
    try:
        gp.fit(X_gp, y_gp)
    except:
        None
...
X_gp=np.concatenate((nprand.uniform(10.0,30.0,[l2,6]),nprand.uniform(-0.3,0.3,[l2,42]),nprand.uniform(0.0,30.0,[l2,1]),nprand.uniform(-30.0,30.0,[l2,1]),nprand.uniform(0.0,0.3,[l2,6]),nprand.uniform(5.0,20.0,[l2,6])),axis=1)
y_gp=map(lambda x: f1(x), X_gp)
gp.fit(X_gp,y_gp)
...
while(1):
    ...
    y_pred1, sigma2_pred1 = gp.predict(np.asarray(parents[0][0]).reshape(1,-1), eval_MSE=True)
    print str(f(parents[0][0]))+" "+str(y_pred1)+" "+str(sigma2_pred1**0.5)
    Xa=Xa2
    Sa=Sa2
